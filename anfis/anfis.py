...
class ANFIS:
    ...
    def trainHybridJangOffLine(self, epochs=5, tolerance=0.1, initialGamma=1000, k=0.01):
        ...
        while (epoch < epochs) and (convergence is not True):
            ...
            #error
            error = np.mean(np.abs(self.Y-layerFive.T))
            average_error = np.mean(np.absolute(self.Y-layerFive.T))
        ...
        return self.fittedValues, average_error
    ...
    def predict(self, testX):
        return predict(self, testX)

    def calculatePredictedError(self, testY, predictions):
        return np.average(np.abs(testY-predictions))
